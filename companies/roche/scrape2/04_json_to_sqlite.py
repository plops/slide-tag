...
skills_table.create({"skill_id": int, "skill_name": str}, pk="skill_id")

education_table.create(
    {"education_id": int, "job_id": str, "degree_level": str, "field_of_study": str},
    pk="education_id",
...
job_locations_table.create(
    {"job_id": str, "location_id": int},
    pk=("job_id", "location_id"),
    foreign_keys=[
        ("job_id", "Jobs", "job_id"),
...
job_skills_table.create(
    {"job_id": str, "skill_id": int},
    pk=("job_id", "skill_id"),
    foreign_keys=[("job_id", "Jobs", "job_id"), ("skill_id", "Skills", "skill_id")],
)
...
def process_job(job_data):
    ...
    description = job_data.get("structureData", {}).get("description") or job_data.get(
        "description"
    )
    date_posted = (
        job_data.get("datePosted", "").split("T")[0]
    ...
    try:
        ...
        if (
            ...
            and "The expected salary range for this position" in job_data["description"]
        ):
            salary_range_str = (
                job_data["description"]
                .split(
                    "The expected salary range for this position based on the primary location of South San Francisco, CA is "
                )[1]
                .split(" USD Annual.")[0]
            )
            salary_parts = (
                salary_range_str.replace("$", "").replace(",", "").split(" - ")
            )
    ...
    target_hire_date = (
        additional_fields.get("targetHireDate", "").split("T")[0]
        if additional_fields.get("targetHireDate")
        else None
    )
    ...
    recruiting_start_date = (
        additional_fields.get("recruitingStartDate", "").split("T")[0]
        if additional_fields.get("recruitingStartDate")
        else None
    )
    ...
    for location in job_data.get("standardised_multi_location", []):
        ...
        inserted_loc = next(
            locations_table.rows_where(
                "city = :city AND state = :state AND country = :country", loc_record
            )
        )
        ...
        job_locations_table.insert(
            {"job_id": job_id, "location_id": location_id}, ignore=True
        )
...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Insert one or more job JSON files into jobs_minutils.db"
    )
    ...
    for fp in args.files:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                job_data = (
                    (data.get("jobDetail", {}).get("data", {}).get("job"))
                    if data.get("jobDetail")
                    else data.get("job") or data.get("job_data") or data
                )
            if not job_data:
